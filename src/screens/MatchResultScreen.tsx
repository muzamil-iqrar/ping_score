...
export default function MatchResultScreen({ navigation, route }: any) {
  ...
  return <ScrollView style={styles.container} contentContainerStyle={styles.content}>
    ...
    <Reveal delay={420} style={{ width: '100%' }}><Touch style={ui.primary} onPress={() => navigation.reset({ index: 1, routes: [{ name: 'MainTabs' }, tournamentId ? { name: 'Tournament', params: { tournamentId } } : { name: 'NewMatch' }] })}><Text style={ui.primaryText}>{tournamentId ? 'Back to tournament  →' : 'Run it back  →'}</Text></Touch><Touch style={styles.homeButton} onPress={() => navigation.reset({ index: 0, routes: [{ name: 'MainTabs' }] })}><Text style={styles.homeText}>Back to home</Text></Touch></Reveal>
  </ScrollView>;
}
